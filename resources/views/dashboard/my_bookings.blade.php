...
@section('content')
    <section id="content" class="w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
          ...
          <a href="{{ route('dashboard.booking.details', $booking->id) }}" class="card">
            <div class="bg-white p-4 rounded-[26px] flex items-center gap-4">
              <p class="text-center text-sm leading-[22px] tracking-035">
                ...
                <br> {{ $booking->start_date->format('M') }} <br>{{ $booking->start_date->format('y') }}</p>
        ...
        <div class="navigation-bar fixed bottom-0 z-50 w-full h-[85px] bg-white rounded-t-[25px] flex items-center justify-evenly py-[45px]">
