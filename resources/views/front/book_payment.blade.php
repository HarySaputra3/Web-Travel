...
@section('content')
    <section id="content" class=" w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
        ...
        <form enctype="multipart/form-data" action="{{ route('front.book_payment_store', $packageBooking->id) }}" method="POST" class="flex flex-col gap-8">
              ...
              <div class="flex flex-col gap-1">
                ...
                <div class="flex gap-1 items-center">
                  ...
                  <span class="text-darkGrey text-sm tracking-035 leading-[22px]">
                    {{ $packageBooking->start_date->format('M d, Y') }} - {{ $packageBooking->end_date->format('M d, Y') }}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="bg-white p-[16px_24px] rounded-[26px] flex flex-col gap-4">
              ...
              <div class="flex flex-col gap-1">
                ...
                <div class="flex items-center justify-between">
                  <span id="total-payment" class="font-semibold text-lg tracking-[0.6px] leading-[26px]" data-value="2500000">
                    Rp. {{ number_format($packageBooking->total_amount, 0, '.', '.') }}</span>
          ...
          <div class="navigation-bar fixed bottom-0 z-50 w-full h-[85px] bg-black rounded-t-[25px] flex items-center justify-between px-6 shadow-[-6px_-8px_20px_0_#00000008]">
