...
@section('content')
    <section
      id="content"
      class="w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[100px]">
      <nav class="mt-8 px-4 w-full flex items-center justify-between">
        ...
        <div class="flex flex-col gap-1">
            <p class="text-xs tracking-035">Welcome!</p>
            <p class="font-semibold">Siap Travelling ?</p>
        </div>
          @endguest
      ...
      <h1 class="font-semibold text-2xl leading-[36px] text-center">
        Explore New<br />Experience with Us
      </h1>
      <div id="categories" class="flex flex-col gap-3 text center">
      ...
      <div id="recommendations" class="flex flex-col gap-3">
        ...
        @forelse ($package_tours as $tour )
          <a href="{{ route('front.details', $tour->slug)  }}""class="group px-2 first-of-type:pl-4 last-of-type:pr-4">
            ...
            >
              <div
                class="w-full h-[330px] rounded-xl flex shrink-0 overflow-hidden"
              >
                <img
                ...
                <div class="flex flex-col gap-1 text-right">
                  <p class="text-sm leading-[21px]">
                    <span class="font-semibold text-[#4D73FF] text-nowrap"
                      >Rp.{{ number_format($tour->price, 0, ',', '.') }}</span><br />
      ...
      <div id="discover" class="px-4">
        ...
        >
          ...
          />
          <div
            class="absolute z-10 flex flex-col gap-[10px] transform -translate-y-1/2 top-1/2 left-4"
          >
            <p class="text-white font-semibold">
              Discover the<br />Beauty of Japan
            </p>
            <a
              href=""
              class="bg-[#4D73FF] p-[8px_24px] rounded-[10px] text-white font-semibold text-xs w-fit"
              >Discover</a
            >
          </div>
        </div>
      ...
      <div id="explore" class="flex flex-col px-4 gap-3">
        ...
        <a href="{{ route('front.details', $tour->slug)  }}" class="card">
          <div
            class="bg-white p-4 flex flex-col gap-3 rounded-[26px] shadow-[6px_8px_20px_0_#00000008]"
          >
            <div
              class="w-full h-full aspect-[311/150] rounded-xl overflow-hidden"
            >
              <img
                src="{{ Storage::url($tour->thumbnail) }}"
                class="w-full h-full object-cover object-center"
            ...
            <div class="flex justify-between gap-2">
              ...
              <div class="flex flex-col gap-1 text-right">
                <p class="text-sm leading-[21px]">
                  <span class="font-semibold text-[#4D73FF] text-nowrap"
                    >Rp.{{ number_format($tour->price, 0, ',', '.') }}</span>
                    <br />
                  <span class="text-darkGrey">{{ $tour->days }} days</span>
                </p>
                <div class="flex items-center gap-1 justify-end">
              ...
              <div class="flex flex-col gap-1 text-right">
                <p class="text-sm leading-[21px]">
                  <span class="font-semibold text-[#4D73FF] text-nowrap"
                    >Rp.{{ number_format($tour->price, 0, ',', '.') }}</span
                  ><br />
                  <span class="text-darkGrey">{{ $tour->days }} days| {{ $tour->quantity }} packss</span>
                </p>
                <div class="flex items-center gap-1 justify-end">
                  <div class="w-4 h-4 flex shrink-0">
                    <img src="assets/icons/Star.svg" alt="icon" />
      ...
      <div
        class="navigation-bar fixed bottom-0 z-50 max-w-[1080px] w-full h-[55px] bg-white rounded-t-[25px] flex items-center justify-evenly py-[45px]"
      >
        <a href="" class="menu">
          <div class="flex flex-col justify-center w-fit gap-1">
            <div
              class="w-4 h-4 flex shrink-0 overflow-hidden mx-auto text-[#4D73FF]"
            >
